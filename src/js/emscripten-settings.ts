...
function getInstantiateWasmFunc(
  ...
  return function (
    ...
    (async function () {
      ...
      // Allow user code (e.g. BridgedFS) to modify imports before instantiation
      if (API.config.onBeforeWasmInstantiate) {
        await API.config.onBeforeWasmInstantiate({ imports });
      }
